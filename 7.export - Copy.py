...
def exec_export(*event):
    ...
    query="SELECT A.ID,B.Employee_Name,B.Employee_DOJ,B.Employee_Position,B.Employee_Department,B.Employee_Grade,B.Employee_Location,B.Employee_Calendar,A.Date,A.Leave_status,C.Email_Id FROM `att_report` A,`erp_report` B, `email` C WHERE A.ID=B.E_code AND A.ID=C.Emp_Code;"
